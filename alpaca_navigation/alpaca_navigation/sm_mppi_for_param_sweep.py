...
class SMMPPIController:
    def __init__(self,static_obs, device, horizon_length=HORIZON_LENGTH, dt=DT, num_samples=NUM_SAMPLES, cov_x = 1.0, cov_z = 1.0, w_action_seq_cost= 10, lambda_ = 1e-2, goal_weight = 1000, terminal_goal_weight = 1000):
        ...
        cov[0, 0] = cov_x
        cov[1, 1] = 1e-8  # keep tiny noise on lateral term
        cov[2, 2] = cov_z
        # MPPI initialization
        ...
        self.mppi = mppi.SMPPI(
            ...
            u_max=torch.tensor([0.1, 0.0, 0.1], dtype=torch.float32).to(self.device),
    ...
    def terminal_cost(self, state: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
        ...
        costmap_weight = 0 #1
        cv_weight = 100
        terminal_goal_weight = self.terminal_goal_weight
